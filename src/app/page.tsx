...
import { DashlyticsToast } from "./lib/toast";
import { key } from "./lib/uniqueKey";
import { isLoading, setIsLoading } from "./store";

export default function Home() {
  const dispatch = useDispatch();
  const loading = useSelector(isLoading);
  useEffect(() => {
    DashlyticsToast.dismiss();
    setTimeout(() => {
                  ...
                  <Card key={key()} className="h-[180px] p-3">
                    <Flex>
                      {loading ? (
                        ...
                        <>
                          ...
                          <Image
                            src={
                              prev > value ? TrendDownTwoIcon : TrendUpTwoIcon
                            }
                            alt="Card Icons"
                            width={104}
                            height={32}
                    ...
                    {loading ? (
                      <div className="flex flex-col pt-2">
                        {[
                          "h-4 w-1/2 grow my-1",
                          "h-6 w-full grow my-1",
                          "h-8 w-full grow my-1",
                        ].map((i: string) => {
                          return (
                            <SkeletonLoader
                              key={key()}
